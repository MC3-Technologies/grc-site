...
interface AdminStats {
  users: { total: number; active: number; pending: number; rejected: number; suspended: number; };
  assessments: { total: number; inProgress: number; completed: number; compliant: number; nonCompliant: number; };
...
const sendEmailHelper = async ({ to, subject, message }: EmailOptions): Promise<boolean> => {
  ...
  try {
    ...
    const command = new SendEmailCommand({
      ...
      Source: env.EMAIL_SENDER || process.env.EMAIL_SENDER || "no-reply-grc@mc3technologies.com", // Fallback
    });
...
const createAuditLogEntry = async (logEntry: Omit<AuditLog, "id">) => {
  // (Keep existing audit log creation logic)
   try {
    ...
    const command = new PutItemCommand({ TableName: getAuditLogTableName(), Item: item });
...
export const userOperations = {
  ...
  listUsers: async (): Promise<string> => {
    try {
      ...
      console.log(`[listUsers] Fetching users from Cognito pool: ${userPoolId}`);
      ...
      do {
        ...
        const response: import('@aws-sdk/client-cognito-identity-provider').ListUsersCommandOutput = await cognito.send(listCommand);
        allCognitoUsers = allCognitoUsers.concat(response.Users || []);
        paginationToken = response.PaginationToken;
      } while (paginationToken);

      console.log(`[listUsers] Fetched ${allCognitoUsers.length} users from Cognito. Enriching with DynamoDB status from table: ${userStatusTableName}`);

      const enrichedUsers = await Promise.all(
        allCognitoUsers.map(async (cognitoUser) => {
          const attributes: Record<string, string> = {};
          cognitoUser.Attributes?.forEach((attr) => { if (attr.Name && attr.Value) attributes[attr.Name] = attr.Value; });
          ...
          let registrationDate: string | undefined = cognitoUser.UserCreateDate?.toISOString(); // Default to Cognito creation date
          ...
          if (email) { // Only query DynamoDB if we have an email
            try {
              ...
              if (dbResponse.Item) {
                  const dbUser = unmarshall(dbResponse.Item) as Partial<UserStatus>;
                  ...
                  if (dbUser.registrationDate) registrationDate = dbUser.registrationDate;
                  if (dbUser.lastStatusChange) lastStatusChange = dbUser.lastStatusChange; 
              }
            } catch (err) { 
                // Log if DynamoDB query fails, but don't block the process
                console.warn(`[listUsers] Could not fetch DynamoDB UserStatus for ${email}: ${(err as Error).message}`); 
            }
          }

          // Determine final values, prioritizing Cognito then DynamoDB for profile, but DynamoDB for status/role
          const finalFirstName = attributes["given_name"] || attributes["custom:firstName"] || dbFirstName;
          const finalLastName = attributes["family_name"] || attributes["custom:lastName"] || dbLastName;
          const finalCompanyName = attributes["custom:companyName"] || dbCompanyName;
          const finalStatus = dbStatus || (cognitoUser.Enabled ? 'active' : 'pending'); // Fallback status based on Cognito enabled state
          const finalRole = dbRole || (attributes["custom:role"] as UserStatus['role']) || 'user'; // Fallback role
    ...
    // (Keep reverted logic using DynamoDB GSI)
     try {
      ...
      const users = (response.Items || []).map((item) => {
        ...
        return {
          email: parsed.email, status: parsed.status, role: parsed.role || "user",
          created: parsed.registrationDate, lastModified: parsed.lastStatusChange,
          enabled: parsed.status !== 'suspended' && parsed.status !== 'rejected',
          firstName: parsed.firstName, lastName: parsed.lastName, companyName: parsed.companyName,
        };
      });
      console.log(`Found ${users.length} users in DynamoDB with status: ${requestedStatus}`);
      return JSON.stringify(users);
    } catch (error) {
      console.error(`Error in getUsersByStatus (${status}):`, error);
      return JSON.stringify({ error: `Failed to retrieve users by status: ${status}` });
    }
  },
  ...
  getUserDetails: async (email: string): Promise<string> => {
    // (Keep existing getUserDetails logic)
     try {
      const userPoolId = getUserPoolId();
      const command = new AdminGetUserCommand({ UserPoolId: userPoolId, Username: email });
      ...
      let dbUser: Partial<UserStatus> = {};
       try {
            const dbResponse = await dynamodb.send(new GetItemCommand({ TableName: getUserStatusTableName(), Key: marshall({ id: email }) }));
            if (dbResponse.Item) dbUser = unmarshall(dbResponse.Item) as Partial<UserStatus>;
          } catch (err) { console.warn(`No DynamoDB UserStatus record found for ${email} in getUserDetails`); }

      const userDetails = {
        email, cognitoStatus: response.UserStatus, enabled: response.Enabled,
        created: response.UserCreateDate?.toISOString(), lastModified: response.UserLastModifiedDate?.toISOString(),
        attributes, status: dbUser.status || 'pending', role: dbUser.role || attributes["custom:role"] || "user",
        firstName: dbUser.firstName || attributes["given_name"], lastName: dbUser.lastName || attributes["family_name"],
        companyName: dbUser.companyName || attributes["custom:companyName"],
        registrationDate: dbUser.registrationDate, lastStatusChange: dbUser.lastStatusChange,
      };
  ...
  // Approve user
  approveUser: async (email: string, adminEmail: string = "system"): Promise<boolean> => {
    // (Keep existing approveUser logic - already updated)
     try {
      const userPoolId = getUserPoolId();
      console.log(`Approving user ${email} by admin: ${adminEmail}`);
      await cognito.send(new AdminEnableUserCommand({ UserPoolId: userPoolId, Username: email }));
      await cognito.send(new AdminUpdateUserAttributesCommand({ UserPoolId: userPoolId, Username: email, UserAttributes: [{ Name: "custom:status", Value: "ACTIVE" }] }));
      await cognito.send(new AdminAddUserToGroupCommand({ UserPoolId: userPoolId, Username: email, GroupName: "Approved-Users" }));

      let profileData: { firstName?: string; lastName?: string; companyName?: string; role?: string } = {}; // Add role here
      ...
      try {
          ...
          if (userDetailsResponse.UserAttributes) {
              ...
              profileData.firstName = fetchedAttributes['given_name'] || fetchedAttributes['custom:firstName']; // Check standard then custom
              profileData.lastName = fetchedAttributes['family_name'] || fetchedAttributes['custom:lastName']; // Check standard then custom
              ...
              console.log(`[approveUser] Mapped profile data for ${email}:`, JSON.stringify(profileData));
      ...
      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_APPROVED", performedBy: adminEmail, affectedResource: "user", resourceId: email, details: { email, approvedAt: new Date().toISOString() } });
      await sendEmailHelper({ to: email, subject: "Welcome to MC3 GRC Platform - Your Account is Approved", message: approvalTemplate() });
      return true;
    } catch (error) { console.error(`Error approving user ${email}:`, error); return false; }
  },

  // Reject user
  rejectUser: async (email: string, reason?: string, adminEmail: string = "system"): Promise<boolean> => {
    // (Keep existing rejectUser logic)
     try {
      const userPoolId = getUserPoolId();
      console.log(`Rejecting user ${email} by admin: ${adminEmail}`);
      await cognito.send(new AdminUpdateUserAttributesCommand({ UserPoolId: userPoolId, Username: email, UserAttributes: [{ Name: "custom:status", Value: "REJECTED" }] }));

      const userStatusUpdateData: Partial<UserStatus> = { id: email, email, status: "rejected", lastStatusChange: new Date().toISOString(), lastStatusChangeBy: adminEmail, rejectionReason: reason };
       await dynamodb.send(new PutItemCommand({ TableName: getUserStatusTableName(), Item: marshall(userStatusUpdateData, { removeUndefinedValues: true }) }));
      console.log(`Updated UserStatus for ${email} to rejected.`);

      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_REJECTED", performedBy: adminEmail, affectedResource: "user", resourceId: email, details: { email, reason: reason || "No reason provided", rejectedAt: new Date().toISOString() } });
      if (reason) await sendEmailHelper({ to: email, subject: "MC3 GRC Platform - Account Request Status", message: rejectionTemplate({ reason }) });
      return true;
    } catch (error) { console.error(`Error rejecting user ${email}:`, error); return false; }
  },

  // Suspend user
  suspendUser: async (email: string, reason?: string, adminEmail: string = "system"): Promise<boolean> => {
    // (Keep existing suspendUser logic)
     try {
      const userPoolId = getUserPoolId();
      console.log(`Suspending user ${email} by admin: ${adminEmail}`);
      await cognito.send(new AdminDisableUserCommand({ UserPoolId: userPoolId, Username: email }));
      await cognito.send(new AdminUpdateUserAttributesCommand({ UserPoolId: userPoolId, Username: email, UserAttributes: [{ Name: "custom:status", Value: "SUSPENDED" }] }));

       const userStatusUpdateData: Partial<UserStatus> = { id: email, email, status: "suspended", lastStatusChange: new Date().toISOString(), lastStatusChangeBy: adminEmail, suspensionReason: reason };
       await dynamodb.send(new PutItemCommand({ TableName: getUserStatusTableName(), Item: marshall(userStatusUpdateData, { removeUndefinedValues: true }) }));
      console.log(`Updated UserStatus for ${email} to suspended.`);

      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_SUSPENDED", performedBy: adminEmail, affectedResource: "user", resourceId: email, details: { email, reason: reason || "No reason provided", suspendedAt: new Date().toISOString() } });
      if (reason) await sendEmailHelper({ to: email, subject: "MC3 GRC Platform - Account Status Update", message: suspensionTemplate({ reason }) });
      return true;
    } catch (error) { console.error(`Error suspending user ${email}:`, error); return false; }
  },

  // Reactivate user
  reactivateUser: async (email: string, adminEmail: string = "system"): Promise<boolean> => {
    // (Keep existing reactivateUser logic)
     try {
      ...
      try {
        const dbResponse = await dynamodb.send(new GetItemCommand({ TableName: getUserStatusTableName(), Key: marshall({ id: email }) }));
        if (dbResponse.Item) currentDbStatus = (unmarshall(dbResponse.Item) as UserStatus).status;
      } catch (err) { console.warn(`Could not get current DB status for ${email}`); }

      await cognito.send(new AdminEnableUserCommand({ UserPoolId: userPoolId, Username: email }));
      await cognito.send(new AdminUpdateUserAttributesCommand({ UserPoolId: userPoolId, Username: email, UserAttributes: [{ Name: "custom:status", Value: "ACTIVE" }] }));

      const userStatusUpdateData: Partial<UserStatus> = { id: email, email, status: "active", lastStatusChange: new Date().toISOString(), lastStatusChangeBy: adminEmail, suspensionReason: undefined, rejectionReason: undefined };
       await dynamodb.send(new PutItemCommand({ TableName: getUserStatusTableName(), Item: marshall(userStatusUpdateData, { removeUndefinedValues: true }) }));
       console.log(`Updated UserStatus for ${email} to active.`);

      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_REACTIVATED", performedBy: adminEmail, affectedResource: "user", resourceId: email, details: { email, previousStatus: currentDbStatus || 'unknown', newStatus: "active", reactivatedAt: new Date().toISOString() } });
      await sendEmailHelper({ to: email, subject: "MC3 GRC Platform - Account Reactivated", message: reactivationTemplate() });
      return true;
    } catch (error) { console.error(`Error reactivating user ${email}:`, error); return false; }
  },

  // Create user
  createUser: async (
    email: string, role: string, shouldSendEmail: boolean = true, performedBy?: string,
    firstName?: string, lastName?: string, companyName?: string
  ): Promise<{ success: boolean; user?: any; error?: string }> => {
    // (Keep existing createUser logic, correcting MessageActionType)
     try {
      const userPoolId = getUserPoolId();
      console.log(`Creating Cognito user ${email} with role ${role} by admin: ${performedBy}`);
      const tempPassword = generateSecurePassword();
      const userAttributes: AttributeType[] = [
        { Name: "email", Value: email }, { Name: "email_verified", Value: "true" },
        { Name: "custom:role", Value: role },
        ...(firstName ? [{ Name: "given_name", Value: firstName }, { Name: "custom:firstName", Value: firstName }] : []),
        ...(lastName ? [{ Name: "family_name", Value: lastName }, { Name: "custom:lastName", Value: lastName }] : []),
        ...(companyName ? [{ Name: "custom:companyName", Value: companyName }] : []),
      ];

      const createCommand = new AdminCreateUserCommand({
        UserPoolId: userPoolId, Username: email, UserAttributes: userAttributes,
        TemporaryPassword: tempPassword,
        // Omit MessageAction to allow default Cognito invitation email when shouldSendEmail is true
        // Set to SUPPRESS only when shouldSendEmail is false
        MessageAction: shouldSendEmail ? undefined : "SUPPRESS",
      });
      const response = await cognito.send(createCommand);

      // DynamoDB record creation is handled by trigger
      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_CREATED", performedBy: performedBy || "system", affectedResource: "user", resourceId: email, details: { email, role, sendEmail: shouldSendEmail, createdAt: new Date().toISOString(), firstName, lastName, companyName } });
      return { success: true, user: response.User };
    } catch (error) { console.error(`Error creating user ${email}:`, error); return { success: false, error: (error as Error).message }; }
  },

  // Update user role
  updateUserRole: async (email: string, role: string, adminEmail: string = "system"): Promise<boolean> => {
    // (Keep existing updateUserRole logic, correcting PutItemCommand)
     try {
      ...
      try {
        const userData = await cognito.send(new AdminGetUserCommand({ UserPoolId: userPoolId, Username: email }));
        currentCognitoRole = userData.UserAttributes?.find(a => a.Name === 'custom:role')?.Value || 'user';
      } catch (error) { console.warn(`Error fetching current Cognito role for ${email}:`, error); }

      await cognito.send(new AdminUpdateUserAttributesCommand({ UserPoolId: userPoolId, Username: email, UserAttributes: [{ Name: "custom:role", Value: role }] }));

      // Fetch existing item to merge, or create if not exists
      let existingItem: Partial<UserStatus> = { id: email, email }; // Start with key
       try {
           const getResponse = await dynamodb.send(new GetItemCommand({ TableName: getUserStatusTableName(), Key: marshall({ id: email }) }));
           if (getResponse.Item) {
               existingItem = unmarshall(getResponse.Item) as UserStatus;
           }
       } catch (getError) { console.warn(`Could not fetch existing UserStatus for ${email} during role update.`); }
      ...
      await dynamodb.send(new PutItemCommand({ TableName: getUserStatusTableName(), Item: marshall(updatedItem, { removeUndefinedValues: true }) })); // No merge option needed
      console.log(`Updated role in DynamoDB for ${email} to ${role}`);

      const currentGroupName = currentCognitoRole === "admin" ? "GRC-Admin" : "Approved-Users";
      const newGroupName = role === "admin" ? "GRC-Admin" : "Approved-Users";
      try {
        if (currentGroupName !== newGroupName) {
           try { await cognito.send(new AdminRemoveUserFromGroupCommand({ UserPoolId: userPoolId, Username: email, GroupName: currentGroupName })); } catch (removeError: any) { if (removeError.name !== 'UserNotFoundException' && removeError.message?.indexOf('not a member') === -1) { console.warn(`Could not remove user from group ${currentGroupName}:`, removeError); } }
           await cognito.send(new AdminAddUserToGroupCommand({ UserPoolId: userPoolId, Username: email, GroupName: newGroupName }));
        }
      } catch (groupError) { console.warn(`Error managing Cognito groups for ${email}:`, groupError); }

      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_ROLE_UPDATED", performedBy: adminEmail, affectedResource: "user", resourceId: email, details: { email, newRole: role, previousRole: currentCognitoRole, updatedAt: new Date().toISOString() } });
      return true;
    } catch (error) { console.error(`Error updating role for ${email}:`, error); return false; }
  },

  // Update user profile attributes
  updateUserProfile: async (
    email: string, firstName?: string, lastName?: string, companyName?: string, adminEmail: string = "system"
  ): Promise<boolean> => {
    // (Keep existing updateUserProfile logic, correcting PutItemCommand)
     try {
      ...
      if (firstName !== undefined) { attributesToUpdateCognito.push({ Name: "given_name", Value: firstName }); attributesToUpdateCognito.push({ Name: "custom:firstName", Value: firstName }); }
      if (lastName !== undefined) { attributesToUpdateCognito.push({ Name: "family_name", Value: lastName }); attributesToUpdateCognito.push({ Name: "custom:lastName", Value: lastName }); }
      if (companyName !== undefined) { attributesToUpdateCognito.push({ Name: "custom:companyName", Value: companyName }); }

      if (attributesToUpdateCognito.length > 0) {
        await cognito.send(new AdminUpdateUserAttributesCommand({ UserPoolId: userPoolId, Username: email, UserAttributes: attributesToUpdateCognito }));
        console.log(`Cognito profile attributes updated for ${email}`);
      } else { console.log("No Cognito profile attributes to update."); }
      ...
      if (Object.keys(attributesToUpdateDynamo).length > 3) { // Check if there's more than just key/timestamps
           ...
           } catch (getError) { console.warn(`Could not fetch existing UserStatus for ${email} during profile update.`); }

           const updatedItem = { ...existingItem, ...attributesToUpdateDynamo }; // Merge new data over existing

          await dynamodb.send(new PutItemCommand({ TableName: getUserStatusTableName(), Item: marshall(updatedItem, { removeUndefinedValues: true }) })); // No merge option
          console.log(`DynamoDB profile attributes updated for ${email}`);
      } else { console.log("No DynamoDB profile attributes to update."); }

      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_PROFILE_UPDATED", performedBy: adminEmail, affectedResource: "user", resourceId: email, details: { email, updatedFields: attributesToUpdateCognito.map(a => a.Name), updatedAt: new Date().toISOString() } });
      return true;
    } catch (error) { console.error(`Error updating profile for ${email}:`, error); return false; }
  },
  ...
  getAdminStats: async (): Promise<AdminStats> => {
    try {
      ...
      const userStatusTableName = getUserStatusTableName();
      try {
        console.log(`[getAdminStats] Scanning UserStatus table for total count: ${userStatusTableName}`);
        ...
        console.log(`[getAdminStats] Total users found in DynamoDB: ${totalDynamoUsers}`);
      } catch (error) {
        console.error(`[getAdminStats] Error scanning UserStatus table (${userStatusTableName}) for total count:`, error);
      ...
      try {
        ...
        do {
          ...
          const cognitoResponse: import('@aws-sdk/client-cognito-identity-provider').ListUsersCommandOutput = await cognito.send(listUsersCommand);
      ...
      try {
        ...
        console.log(`[getAdminStats] Fetching assessments. Completed: ${completedTableName}, InProgress: ${inProgressTableName}`);
        ...
        stats.assessments.compliant = completedAssessments.filter((a: any) => a.isCompliant === true).length;
        stats.assessments.nonCompliant = completedAssessments.filter((a: any) => a.isCompliant === false).length;
        stats.complianceRate = stats.assessments.completed > 0
          ? Math.round((stats.assessments.compliant / stats.assessments.completed) * 100)
          : 0;
        console.log(`[getAdminStats] Assessment stats:`, stats.assessments);
          
      } catch (error) {
        console.error("[getAdminStats] Error fetching assessment statistics:", error);
        stats.assessments = { total: 0, inProgress: 0, completed: 0, compliant: 0, nonCompliant: 0 };
      ...
      try {
        ...
        console.log(`[getAdminStats] Fetching recent activity from: ${auditLogTableName}`);
        ...
        let activities = (auditLogResponse.Items || []).map(item => unmarshall(item) as AuditLog);
        // Sort by timestamp descending
        activities.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        stats.recentActivity = activities.slice(0, 50); // Ensure limit
        console.log(`[getAdminStats] Fetched ${stats.recentActivity.length} recent activities.`);
      } catch (error) {
  ...
  getAuditLogs: async (
    ...
    // (Keep existing getAuditLogs logic)
     try {
      ...
      if (dateRange?.startDate) { filterExpressions.push("#ts >= :startDate"); expressionAttributeNames["#ts"] = "timestamp"; expressionAttributeValues[":startDate"] = { S: new Date(dateRange.startDate).toISOString() }; }
      if (dateRange?.endDate) { const endDate = new Date(dateRange.endDate); endDate.setHours(23, 59, 59, 999); filterExpressions.push("#ts <= :endDate"); expressionAttributeNames["#ts"] = "timestamp"; expressionAttributeValues[":endDate"] = { S: endDate.toISOString() }; }
      if (filters?.action) { filterExpressions.push("contains(#action, :actionVal)"); expressionAttributeNames["#action"] = "action"; expressionAttributeValues[":actionVal"] = { S: filters.action }; }
      if (filters?.performedBy) { filterExpressions.push("performedBy = :performedByVal"); expressionAttributeValues[":performedByVal"] = { S: filters.performedBy }; }
      if (filters?.affectedResource) { filterExpressions.push("affectedResource = :affectedResourceVal"); expressionAttributeValues[":affectedResourceVal"] = { S: filters.affectedResource }; }

      if (filterExpressions.length > 0) { scanInput.FilterExpression = filterExpressions.join(" AND "); if (Object.keys(expressionAttributeNames).length > 0) { scanInput.ExpressionAttributeNames = expressionAttributeNames; } scanInput.ExpressionAttributeValues = expressionAttributeValues; }
    ...
    } catch (error) { console.error("Error retrieving audit logs:", error); return JSON.stringify({ error: "Failed to retrieve audit logs" }); }
  },
  ...
  getAllSystemSettings: async (): Promise<string> => {
    // (Keep existing getAllSystemSettings logic)
     try {
      ...
      const command = new ScanCommand({ TableName: getSystemSettingsTableName() });
      const response = await dynamodb.send(command);
      if (!response.Items || response.Items.length === 0) { return JSON.stringify({ settings: [], settingsByCategory: {} }); }
      const settings = response.Items.map(item => unmarshall(item)) as SystemSetting[];
      const settingsByCategory: Record<string, SystemSetting[]> = {};
      settings.forEach((setting) => { const category = setting.category || "general"; if (!settingsByCategory[category]) settingsByCategory[category] = []; settingsByCategory[category].push(setting); });
      return JSON.stringify({ settings, settingsByCategory });
    } catch (error) { console.error("Error retrieving system settings:", error); return JSON.stringify({ error: "Failed to retrieve system settings", settings: [], settingsByCategory: {} }); }
  },

  // Update system settings
  updateSystemSettings: async (
    settings: SystemSetting | SystemSetting[], updatedBy?: string
  ): Promise<string> => {
    // (Keep existing updateSystemSettings logic)
     try {
      ...
      for (const setting of settingsArray) {
        try {
          if (!setting.id) setting.id = `setting-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
          setting.lastUpdated = now; if (updatedBy) setting.updatedBy = updatedBy;
          const item = marshall(setting, { removeUndefinedValues: true });
          await dynamodb.send(new PutItemCommand({ TableName: getSystemSettingsTableName(), Item: item }));
          updatedSettings.push(setting);
          await createAuditLogEntry({ timestamp: now, action: "SYSTEM_SETTING_UPDATED", performedBy: updatedBy || "system", affectedResource: "SystemSettings", resourceId: setting.id, details: { name: setting.name, category: setting.category || "general" } });
        } catch (settingError) { console.error(`Error updating setting ${setting.name}:`, settingError); }
      }
      return JSON.stringify({ success: true, updatedCount: updatedSettings.length, updatedSettings });
    } catch (error) { console.error("Error updating system settings:", error); return JSON.stringify({ success: false, error: "Failed to update system settings" }); }
  },

  // Delete user
  deleteUser: async (email: string, adminEmail: string = "system"): Promise<{ success: boolean; message: string }> => {
    // (Keep existing deleteUser logic)
     try {
      console.log(`Attempting to delete user: ${email} by admin: ${adminEmail}`);
      const userPoolId = getUserPoolId();
      const thirtyDaysInSeconds = 30 * 24 * 60 * 60;
      const ttl = Math.floor(Date.now() / 1000) + thirtyDaysInSeconds;
      const userStatusUpdateData: Partial<UserStatus> = { id: email, email, status: "deleted", lastStatusChange: new Date().toISOString(), lastStatusChangeBy: adminEmail, ttl };
      await dynamodb.send(new PutItemCommand({ TableName: getUserStatusTableName(), Item: marshall(userStatusUpdateData, { removeUndefinedValues: true }) })); // Use PutItem, merge not needed
      console.log(`Marked UserStatus for ${email} as deleted with TTL.`);

      await createAuditLogEntry({ timestamp: new Date().toISOString(), action: "USER_DELETED", performedBy: adminEmail, affectedResource: "user", resourceId: email, details: { email, deletedAt: new Date().toISOString(), deletedBy: adminEmail } });
      // Add assessment cleanup logic here if needed
      try { await cognito.send(new AdminDeleteUserCommand({ UserPoolId: userPoolId, Username: email })); console.log(`Successfully deleted Cognito user: ${email}`); } catch(cognitoDeleteError) { console.error(`Error deleting Cognito user ${email}:`, cognitoDeleteError); }
      return { success: true, message: `User ${email} has been marked for deletion and Cognito user deleted.` };
    } catch (error) { console.error("Error deleting user:", error); return { success: false, message: `Failed to delete user: ${(error as Error).message}` }; }
  },

  // Migrate users from Cognito to DynamoDB
  migrateUsersToDynamoDB: async (): Promise<{ success: boolean; count: number; errors: number }> => {
    try {
      ...
      for (const user of users) {
        ...
        try {
          ...
          const firstName = attributes.given_name || attributes["custom:firstName"];
          const lastName = attributes.family_name || attributes["custom:lastName"];
          ...
          if (attributes["custom:status"] === "ACTIVE" && user.Enabled) {
            status = "active";
          } else if (attributes["custom:status"] === "SUSPENDED" || !user.Enabled) {
            status = "suspended";
          ...
          const userStatusData: Partial<UserStatus> = {
            ...
            registrationDate: user.UserCreateDate?.toISOString() || new Date().toISOString(),
          ...
          // Write to DynamoDB
          console.log(`Writing user ${email} to DynamoDB with status ${status}`);
          await dynamodb.send(
            new PutItemCommand({
