...
const getUserPoolId = (): string | null => {
  try {
    ...
    // Check if the USER_POOL_ID environment variable is set
    if (process.env.USER_POOL_ID) {
      console.log("Using USER_POOL_ID:", process.env.USER_POOL_ID);
      return process.env.USER_POOL_ID;
    }
    
    // Log warning if USER_POOL_ID is missing
    console.warn("USER_POOL_ID is missing, checking fallbacks");
    
    // Try other common environment variable names as fallbacks
    ...
    return "us-west-1_10IP1yz5s"; // Hardcoded fallback as last resort
  } catch (error) {
...
export const userOperations = {
  ...
  // Approve user
  approveUser: async (email: string, adminEmail: string = "admin@example.com"): Promise<boolean> => {
    try {
      const userPoolId = getUserPoolId();
      if (!userPoolId || !cognito) {
        throw new Error("Missing UserPoolId or Cognito client not initialized");
  ...
  // Reactivate user
  reactivateUser: async (email: string, adminEmail: string = "admin@example.com"): Promise<boolean> => {
    try {
      const userPoolId = getUserPoolId();
      if (!userPoolId) {
        console.error("User pool ID not found");
  ...
  createUser: async (
    ...
    try {
      ...
      console.log(`Creating user ${email} with role ${role} by admin: ${adminEmail}`);
  ...
  // Update user role
  updateUserRole: async (email: string, role: string, adminEmail: string = "admin@example.com"): Promise<boolean> => {
    try {
      const userPoolId = getUserPoolId();
      if (!userPoolId || !cognito) {
        throw new Error("Missing UserPoolId or Cognito client not initialized");
    ...
    try {
        ...
        if (users.length > 0) {
          ...
          users.forEach((user: UserData) => {
            ...
            if (customStatus === "REJECTED") {
              ...
              stats.users.active++;
            } else if (user.status === "FORCE_CHANGE_PASSWORD" && customStatus !== "REJECTED") {
              stats.users.pending++;
            }
          });
        }
      ...
      try {
        ...
        console.log("Environment variables for DynamoDB tables:", {
          COMPLETED_ASSESSMENT_TABLE: process.env.COMPLETED_ASSESSMENT_TABLE_NAME,
          IN_PROGRESS_ASSESSMENT_TABLE: process.env.IN_PROGRESS_ASSESSMENT_TABLE_NAME
        });
        
        // Get completed assessments
        const completedTableName = process.env.COMPLETED_ASSESSMENT_TABLE_NAME || 
        ...
        try {
          ...
          console.log("Completed assessments scan response:", JSON.stringify(completedResponse));
          ...
          console.error("Error scanning completed assessments table:", scanError);
          console.error("Table name used:", completedTableName);
        }
        
        // Get in-progress assessments
        const inProgressTableName = process.env.IN_PROGRESS_ASSESSMENT_TABLE_NAME || 
          'InProgressAssessment-fk4antj52jgh3j6qjhbhwur5qa-NONE';
        console.log("Using in-progress assessments table:", inProgressTableName);
        ...
        try {
          ...
          console.error("Error scanning in-progress assessments table:", scanError);
      ...
      try {
        ...
        if (auditResponse.Items) {
          stats.recentActivity = auditResponse.Items
            .map(item => unmarshall(item) as AuditLog)
  ...
  getAuditLogs: async (
    ...
    try {
      ...
      const startDate = dateRange?.startDate ? new Date(dateRange.startDate) : defaultStartDate;
      ...
      if (filters) {
        ...
        if (filters.performedBy) {
          logs = logs.filter(log => log.performedBy.includes(filters.performedBy!));
        }
        
        if (filters.affectedResource) {
          logs = logs.filter(log => log.affectedResource.includes(filters.affectedResource!));
        }
      }
    ...
    try {
      ...
      for (const setting of settingsArray) {
        try {
          ...
          console.error(`Error updating setting ${setting.name}:`, settingError);
        }
      }
  ...
  // Delete user
  deleteUser: async (email: string, adminEmail: string = "admin@example.com"): Promise<{success: boolean, message: string}> => {
    try {
      console.log(`Attempting to delete user with email: ${email} by admin: ${adminEmail}`);
      ...
      try {
        ...
        const inProgressAssessmentTableName = process.env.API_GRCSITE2_INPROGRESSASSESSMENTTABLE_NAME;
        if (inProgressAssessmentTableName) {
          ...
          console.log(`Found ${inProgressAssessments.Items?.length || 0} in-progress assessments for user`);
          
          // Delete each assessment
          if (inProgressAssessments.Items && inProgressAssessments.Items.length > 0) {
        ...
        const completedAssessmentTableName = process.env.API_GRCSITE2_COMPLETEDASSESSMENTTABLE_NAME;
