...
export const handler = async (event: AppSyncEvent): Promise<any> => {
  log.info("User management handler received event:", JSON.stringify(event, null, 2));
  ...
  log.info(`Executing operation: ${operation} with args: ${JSON.stringify(event.arguments)}`);
