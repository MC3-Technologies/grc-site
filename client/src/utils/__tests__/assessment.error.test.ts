...
const getClientSchema = originalGetClientSchema as jest.MockedFunction<typeof originalGetClientSchema>;
...
describe.skip("Assessment Error Handling", () => { // Skip the entire suite for now
  ...
  describe("InProgressAssessment", () => {
    ...
    test("updateAssessment should handle errors in _fetchAssessmentStoragePath", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockResolvedValue({ errors: [{ message: "DB Error" }], data: null });
    ...
    */
    ...
    test("updateAssessment should handle errors in _updateAssessmentEntry", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockResolvedValue({ errors: null, data: { storagePath: "test/path.json" } });
      mockClient.models.InProgressAssessment.update.mockResolvedValue({ errors: [{ message: "Update error" }], data: null });
    ...
    test("deleteAssessment should handle errors in _deleteAssessmentEntry", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockResolvedValue({ errors: null, data: { storagePath: "test/path.json" } });
      mockClient.models.InProgressAssessment.delete.mockRejectedValue(new Error("Delete error"));
    ...
    test("deleteAssessment should handle errors in _deleteAssessmentFromStorage", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockResolvedValue({ errors: null, data: { storagePath: "test/path.json" } });
      mockClient.models.InProgressAssessment.delete.mockResolvedValue({ errors: null, data: { id: "test-id" } });
      // Use direct mock rejection
    ...
    test("fetchAssessmentData should handle errors correctly", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockRejectedValue(new Error("Network Error"));
    ...
    test("fetchAssessmentData should handle no data returned", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockResolvedValue({ errors: null, data: null });
    ...
    test("fetchAssessmentStorageData should handle errors in _fetchAssessmentStoragePath", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockRejectedValue(new Error("DB Error"));
    ...
    */
    ...
    test("fetchAllAssessments should handle errors correctly", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.list.mockRejectedValue(new Error("List error"));
    ...
    */
    ...
    test("createAssessment should handle errors in _createAssessmentEntry", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.create.mockRejectedValue(new Error("Create error"));
      // Ensure upload succeeds
      mockedUploadData.mockResolvedValue({} as unknown); // Mock successful upload
  ...
  describe("CompletedAssessment", () => {
    ...
    test("completeInProgressAssessment should handle errors in _uploadAssessmentToStorage", async () => {
      // Arrange
      mockClient.models.InProgressAssessment.get.mockResolvedValue({ errors: null, data: { id: "test-id", name: "Test", startedAt: new Date().toISOString(), storagePath: "old/path.json" } });
      ...
      // Act & Assert
      try {
        await CompletedAssessment.completeInProgressAssessment(testFile, "test-id");
        // If it doesn't throw, fail the test
        throw new Error("Test failed: Expected function to throw.");
      } catch (error) {
        expect(error.message).toBe("Error uploading completed assessment JSON to storage"); // Check the specific error message
      }
    });

    test("deleteAssessment should handle errors correctly", async () => {
      // Arrange
      mockClient.models.CompletedAssessment.get.mockRejectedValue(new Error("Database error"));
    ...
    test("fetchAssessmentData should handle no data returned", async () => {
      // Arrange
      mockClient.models.CompletedAssessment.get.mockResolvedValue({ errors: null, data: null });
    ...
    test("fetchAllCompletedAssessments should handle errors correctly", async () => {
      // Arrange
      mockClient.models.CompletedAssessment.list.mockRejectedValue(new Error("List error"));
  ...
  describe("Exported Assessment Functions Error Handling", () => {
    ...
    describe("listAssessments", () => {
      it("should handle list errors", async () => {
        ...
        // Ensure console.error spy is checked correctly
        expect(console.error).toHaveBeenCalledWith("Error listing assessments:", expect.any(Error));
      });
    ...
    describe("createAssessment", () => {
      it("should handle save errors", async () => {
         ...
         expect(result.success).toBe(false);
         expect(result.message).toContain("Error uploading new assessment to storage"); // Keep this as it matches the internal function's throw
      });
    ...
    describe("updateAssessment", () => {
      it("should handle save errors", async () => {
        ...
        jest.spyOn(assessment, 'getAssessment').mockResolvedValueOnce({ name: "Old Name" });
        // Use direct mock rejection
        mockedUploadData.mockRejectedValueOnce(new Error("Save error"));
        // Corrected: Removed 'title' property
        const result = await assessment.updateAssessment("test-id", { name: "Updated Name" });
        expect(result.success).toBe(false);
