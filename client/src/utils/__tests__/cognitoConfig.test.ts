...
jest.mock("../cognitoConfig", () => {
  ...
  const getEnvConfig = () => ({
    userPoolId:
      process.env.VITE_USER_POOL_ID || process.env.REACT_APP_USER_POOL_ID || "",
    region:
      process.env.VITE_AWS_REGION || process.env.REACT_APP_AWS_REGION || "",
    clientId:
      process.env.VITE_CLIENT_ID || process.env.REACT_APP_CLIENT_ID || "",
    identityPoolId:
      process.env.VITE_IDENTITY_POOL_ID ||
  ...
  return {
    getCognitoConfig: jest.fn().mockImplementation(() => {
      ...
      const config = {
        ...
        identityPoolId:
          envConfig.identityPoolId || amplifyConfig.identityPoolId || "",
      };
