import { getLatestQuestionnaireData, loadSavedQuestionnaire, QUESTIONNAIRE_STORAGE_KEY, QuestionPage } from '../questionnaireUtils';
...
describe('Questionnaire Utilities', () => {
  ...
  describe('loadSavedQuestionnaire', () => {
    test('returns null when no data in localStorage', () => {
      ...
      expect(localStorageMock.getItem).toHaveBeenCalledWith(QUESTIONNAIRE_STORAGE_KEY);
    });
    ...
    test('returns parsed data from localStorage when available', () => {
      ...
      localStorageMock.setItem(QUESTIONNAIRE_STORAGE_KEY, JSON.stringify(testPages));
  ...
  describe('getLatestQuestionnaireData', () => {
    ...
    test('returns modified surveyJson with pages from localStorage when available', () => {
      ...
      expect((result.pages[0] as unknown as { id?: string }).id).toBeUndefined();
    });
