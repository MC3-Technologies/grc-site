...
jest.mock('../../amplify/schema', () => ({
  getClientSchema: jest.fn(() => ({
    queries: {
      ...
      // @ts-expect-error - Jest mock implementation with args
      getUsersByStatus: jest.fn().mockImplementation((args: { status: string }) => {
...
describe('Admin User Management API', () => {
  ...
  describe('rejectUser', () => {
    it('should reject a user', async () => {
      const result = await rejectUser('user2@example.com', 'Not a valid request');
      expect(result).toBe(true);
    });
  });
  ...
  describe('suspendUser', () => {
    it('should suspend a user', async () => {
      const result = await suspendUser('user1@example.com', 'Violation of terms');
      expect(result).toBe(true);
    });
  });
