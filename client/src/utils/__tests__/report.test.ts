// THE FOLLOWING TESTS WERE GENERATED BY CHATGPT
// Report.test.ts
...
describe("Report class", () => {
  it("should throw an error if assessment data is null", () => {
    expect(() => new Report(null as unknown as Record<string, string>)).toThrow(
      "Assessment data is null!"
    );
  });

  it("should handle non-object assessment data without throwing and produce empty report", () => {
    const report = new Report(
      "not an object" as unknown as Record<string, string>
    );
    const reportData = report.generateReportData();

    // Since there are no valid '@' keys, the report should be empty
    expect(reportData.score).toBe(0);
    expect(reportData.maxScore).toBe(0);
    expect(reportData.controlGroupResults.size).toBe(0);
  });
